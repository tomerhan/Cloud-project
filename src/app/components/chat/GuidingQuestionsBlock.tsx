...
export default function GuidingQuestionsBlock({ articles, selectedArticleIds, disabled, disabledReason, onQuestionsChange }: Props) {
  ...
  const answerQuestion = async (id: string) => {
    ...
    if (!targetArticleId) {
      toast.error(t('chat.toastSelectMentionArticle'));
      return;
    }
    ...
    <section className="bg-card rounded-2xl shadow-sm border border-border p-5 space-y-5">
      <div className="flex items-start gap-3 border-b border-border pb-4">
        ...
        <div className="flex-1">
          <h2 className="text-lg font-bold text-foreground">{t('chat.guidedQuestionsTitle')}</h2>
          <p className="text-xs text-muted-foreground mt-0.5 font-medium">
            {t('chat.guidedQuestionsSubtitle')}
          </p>
        </div>
      </div>
          ...
          >
            <header className="px-4 py-2.5 bg-muted/60 border-b border-border flex items-center justify-between">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                <FileText className="w-3 h-3 text-red-600" /> {t('chat.questionLabel')} {idx + 1}
              </span>
              <button
                onClick={() => remove(q.id)}
                ...
                aria-label={t('chat.removeQuestionAria').replace('{n}', String(idx + 1))}
              >
            ...
            <div className="p-4 space-y-3">
              <div>
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide block mb-1.5">
                  {t('chat.yourQuestionLabel')} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={q.text}
              ...
              <div>
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide block mb-1.5">
                  {t('chat.guideNoteLabel')} <span className="text-muted-foreground/70 normal-case font-medium">{t('chat.optionalLabel')}</span>
                </label>
                <textarea
                  value={q.guide}
                  onChange={(e) => updateGuide(q.id, e.target.value)}
                  disabled={disabled}
                  placeholder={t('chat.guideNotePlaceholder')}
              ...
              {q.mentioned.length > 0 && (
                <div>
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
                    <BookOpen className="w-3 h-3 text-red-600" /> {t('chat.articlesReferenced')}
                  </p>
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-muted-foreground truncate">
                            {a.authors[0]}{a.authors.length > 1 ? ` ${t('chat.etAl')}` : ''} • {a.year}
                          </p>
                        </div>
                      </div>
                    ))}
              ...
              <div className="flex items-center justify-end">
                ...
                >
                  ...
                  {q.loading ? t('chat.thinking') : q.answer ? t('chat.regenerateAnswer') : t('chat.getAnswer')}
                </button>
              </div>
              ...
              {q.answer && !q.loading && (
                <div className="rounded-xl border border-border bg-muted/40 p-3">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide mb-1.5 flex items-center gap-1.5">
                    <Sparkles className="w-3 h-3 text-red-600" /> {t('chat.guidedAnswerLabel')}
                  </p>
      ...
      <div className="flex items-center justify-between gap-3 pt-1 border-t border-border">
        <button
          ...
          aria-label={t('chat.addQuestionAria')}
        >
          <Plus className="w-4 h-4" /> {t('chat.addQuestion')}
        </button>

        <span className="text-xs text-muted-foreground mt-4">
          {questions.filter(q => q.text.trim()).length} {questions.filter(q => q.text.trim()).length === 1 ? t('chat.questionsReadySingular') : t('chat.questionsReadyPlural')}
          {!hasAtLeastOne && questions.length > 0 && <span className="text-red-500 ml-2">{t('chat.fillAtLeastOne')}</span>}
        </span>
      </div>

      {disabled && (
        <p className="text-xs text-muted-foreground italic">
          {disabledReason || t('chat.guidedQuestionsDefaultDisabledReason')}
        </p>
      )}
    </section>
  );
