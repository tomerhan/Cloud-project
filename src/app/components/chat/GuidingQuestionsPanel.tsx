    ...
    <div className="w-80 lg:w-96 bg-card border-l border-border flex flex-col h-full shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-20 overflow-hidden absolute right-0 top-0">
      <div className="p-4 border-b border-border flex items-center justify-between bg-muted/50">
        <h2 className="font-bold text-foreground flex items-center gap-2 text-base">
          ...
          {t('chat.guidingQuestionsPanelTitle')}
        </h2>
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-3 mb-4">
            {questions.map((q, idx) => (
              <div key={idx} className="group p-3.5 bg-card rounded-xl border border-border shadow-sm relative">
                <div className="text-[10px] font-bold text-gray-600 mb-1.5 uppercase tracking-wider">{t('chat.questionLabel')} {idx + 1}</div>
                ...
                <button
                  ...
                  title={t('chat.deleteQuestionTitle')}
                >
          ...
          {isAdding ? (
            <div className="p-3.5 bg-card border border-gray-300 rounded-xl shadow-sm ring-2 ring-gray-50">
              <textarea
                ...
                placeholder={t('chat.typeQuestionPlaceholder')}
            ...
            <div className="p-3.5 bg-card border border-border rounded-xl shadow-sm">
              <h3 className="font-bold text-foreground mb-4">{t('chat.addGuidingQuestionsTitle')}</h3>
              <button
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="font-bold text-foreground text-sm">{t('chat.practiceQATitle')}</h3>
          </div>
          <p className="text-xs text-muted-foreground mb-3 leading-relaxed">
            {t('chat.practiceQADesc')}
          </p>
          ...
          <div className="flex gap-2 mb-3">
            <input
              ...
              placeholder={t('chat.practiceQuestionPlaceholder')}
            ...
            >
              <Send className="w-3.5 h-3.5" /> {t('chat.ask')}
            </button>
          </div>

          {practiceLoading && (
            <div className="text-xs text-muted-foreground mb-3">{t('chat.thinking')}</div>
          )}
            ...
            {practiceLog.map((item, i) => (
              <div key={i} className="rounded-xl border border-border bg-card overflow-hidden">
                <div className="px-3 py-2 bg-amber-50 dark:bg-amber-950/30 border-b border-border">
                  <p className="text-[10px] font-bold text-amber-700 dark:text-amber-300 uppercase tracking-wide mb-0.5">{t('chat.youAsked')}</p>
                  <p className="text-xs text-foreground">{item.q}</p>
                </div>
                <div className="px-3 py-2.5">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide mb-1">{t('chat.guidedAnswerLabel')}</p>
                ...
                <div className="px-3 py-2 border-t border-border bg-muted/50">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide mb-1.5 flex items-center gap-1">
                    <BookOpen className="w-3 h-3" /> {t('chat.followUpPrompts')}
                  </p>
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="font-bold text-foreground text-sm">{t('chat.aiGuidanceTitle')}</h3>
          </div>
          <p className="text-xs text-muted-foreground mb-3 leading-relaxed">
            {t('chat.aiGuidanceDesc')}
          </p>
          <div className="p-4 bg-card border-t border-border">
            <div className="flex items-center gap-2">
              <label className="text-sm font-medium text-foreground mb-1">
                {t('chat.customInstructions')}
              </label>
              <textarea
                value={instruction}
                onChange={(e) => setInstruction(e.target.value)}
                placeholder={t('chat.customInstructionsPlaceholder')}
